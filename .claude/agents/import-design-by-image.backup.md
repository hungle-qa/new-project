...
**Step 4: Embed with Proper Sizing and Comment**
```html
<!-- {filename}.svg -->
...
1. **Run Glob FIRST** to get real-time list: `source/design-system/icons/*.svg`
2. **Search Glob results** for matching keywords in filenames
3. **Read the matched .svg file** to get actual SVG content
4. Embed inline with appropriate Tailwind classes:
   ```html
   <!-- share-white.svg -->
   <svg class="w-5 h-5" ...>{actual SVG content}</svg>
   ```
5. If no matching icon found, note it as "Icon needed: {description}"
...
## HTML
\`\`\`html
{generated HTML code with Tailwind classes - NO sub-headers before this!}
\`\`\`
