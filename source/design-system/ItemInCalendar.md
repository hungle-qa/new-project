    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function checkTruncation() {
        document.querySelectorAll('.item-title').forEach(function(title) {
          var wrapper = title.closest('.title-wrapper');
          var tooltip = wrapper ? wrapper.querySelector('.title-tooltip') : null;
          if (tooltip) {
      ...
      // Native checkbox - stop propagation to prevent card click
      document.querySelectorAll('.custom-checkbox').forEach(function(cb) {
        cb.addEventListener('click', function(e) {
          e.stopPropagation();
        });
      });

      // Helper: close all dropdowns and hide tooltips
      function closeAllDropdowns() {
        document.querySelectorAll('.more-options-dropdown').forEach(function(d) {
          d.classList.add('opacity-0', 'invisible', '-translate-y-2');
          d.classList.remove('opacity-100', 'visible', 'translate-y-0');
        });
        document.querySelectorAll('.more-options-btn').forEach(function(b) {
          b.setAttribute('aria-expanded', 'false');
        });
      }

      // MoreOptions: tooltip on hover, dropdown on click
      document.querySelectorAll('.more-options-wrapper').forEach(function(wrapper) {
        var btn = wrapper.querySelector('.more-options-btn');
        var dropdown = wrapper.querySelector('.more-options-dropdown');
        var tooltip = wrapper.querySelector('.more-options-tooltip-inline');
        var menuItems = dropdown.querySelectorAll('[role="menuitem"]');

        // Show tooltip on hover (only when dropdown closed)
        btn.addEventListener('mouseenter', function() {
          if (dropdown.classList.contains('invisible') && tooltip) {
        ...
        dropdown.addEventListener('keydown', function(e) {
          ...
          var index = Array.prototype.indexOf.call(menuItems, focused);

          if (e.key === 'ArrowDown') {
            e.preventDefault();
            var next = (index + 1) % menuItems.length;
            menuItems[next].focus();
          } else if (e.key === 'ArrowUp') {
            e.preventDefault();
            var prev = (index - 1 + menuItems.length) % menuItems.length;
      ...
      document.querySelectorAll('.more-options-dropdown [role="menuitem"]').forEach(function(item) {
        item.addEventListener('click', function(e) {
          e.stopPropagation();
          var action = this.querySelector('span') ? this.querySelector('span').textContent : '';
          var card = this.closest('.item-in-calendar');
          var title = card ? (card.querySelector('.item-title') ? card.querySelector('.item-title').textContent : '') : '';
          console.log('Action:', action, 'on:', title);
          document.dispatchEvent(new CustomEvent('more-options-action', {
            detail: { action: action, title: title }
          }));
          closeAllDropdowns();
        });
      });
