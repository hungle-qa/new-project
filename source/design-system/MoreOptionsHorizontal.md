...
## Preview
A horizontal three-dot menu button (⋯) that opens a dropdown menu with action items. Features tooltip on hover and dark dropdown menu on click.
...
## Usage
Use for overflow menus, contextual actions on cards/items, or additional options. Shows common actions like Duplicate, Edit, Sharing settings, and Remove.
...
<div class="more-options-wrapper">
  <!-- Three-dot Button -->
  <button class="more-options-btn" id="moreOptionsBtn" aria-haspopup="true" aria-expanded="false">
    <span class="more-options-dots">⋯</span>
  </button>
  ...
  <div class="more-options-dropdown" id="moreOptionsDropdown" role="menu">
    <!-- Duplicate -->
...
## Tailwind CSS
```html
<!-- More Options Horizontal with Tailwind -->
<div class="relative inline-block font-['Open_Sans']" id="moreOptionsWrapper">
  <!-- Button -->
  <button
    id="moreOptionsBtnTailwind"
    ...
    aria-expanded="false"
  >
    <span class="text-[#141414] text-xl tracking-wider leading-none">⋯</span>
  </button>

  <!-- Tooltip -->
  <div
    class="more-options-tooltip-tailwind absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-1.5 bg-[#1F2937] text-white text-xs font-medium rounded-md whitespace-nowrap opacity-0 invisible transition-all pointer-events-none z-[999]"
  >
    More options
  ...
  <!-- Dropdown -->
  <div
    id="moreOptionsDropdownTailwind"
...
## JavaScript
```javascript
// MoreOptionsHorizontal - Toggle Dropdown and Tooltip
document.addEventListener('DOMContentLoaded', function() {
  const wrapper = document.getElementById('moreOptionsWrapper');
  const btn = document.getElementById('moreOptionsBtnTailwind');
  const dropdown = document.getElementById('moreOptionsDropdownTailwind');
  const tooltip = wrapper?.querySelector('.more-options-tooltip-tailwind');
...
|---------|-------------|---------|
| default | Horizontal dots (⋯) | `text-xl tracking-wider` |
...
- Icons embedded inline from design system
