...
export function CreateKnowledgeModal({ isOpen, onClose, onCreated }: CreateKnowledgeModalProps) {
  ...
  const handleCreate = async () => {
    if (!name.trim()) {
      setError('Knowledge name is required')
      return
    }
    ...
    try {
      const res = await fetch('/api/feature-knowledge', {
        method: 'POST',
      ...
      <div className="relative min-h-screen flex items-center justify-center p-4">
        <div className="relative bg-white rounded-lg shadow-xl w-full max-w-md">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <h2 className="text-lg font-semibold text-gray-900">Create Knowledge Item</h2>
          ...
          <div className="p-6 space-y-4">
            <div>
              <label htmlFor="knowledgeName" className="block text-sm font-medium text-gray-700 mb-1">
                Knowledge Name
              </label>
              <input
                type="text"
                ...
                placeholder="e.g. api-guidelines, ux-principles"
