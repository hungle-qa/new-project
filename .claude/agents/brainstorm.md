...
|--------|---------|-----------|
| `app-build` | UI/UX, logic, backend/frontend architecture, database schema, web/mobile app | `.claude/skills/brainstorm/app-build.md` |
| `ai-workflow` | Prompt chaining, RAG, agentic frameworks, LLM, API cost optimization, AI pipeline | `.claude/skills/brainstorm/ai-workflow.md` |
| `hybrid` | App with significant AI components, AI-powered features in a product | Read BOTH skills, merge relevant sections |

After classification, read `.claude/skills/brainstorm/shared.md` first (complexity classification + step gate), then read the corresponding skill file(s) and follow the workflow defined there.
